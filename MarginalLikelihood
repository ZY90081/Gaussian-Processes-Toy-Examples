function x = MarginalLikelihood(D,sigma)
...
x = [1,0];    % initial guess
fvals(iter) = neglog_marginal_likelihood(D,x,sigma);
fprintf('iter = %3d: x = %-10s, neg_log_likelihood = %f\n', ...
    iter, mat2str(x,6), fvals(iter));
while iter < MAX_ITER && fvals(end) > FUNC_TOL
    iter = iter + 1;
    x = x - GAMMA * neg_partiallog_marginal_likelihood(D,x,sigma);  % gradient descent
    fvals(iter) = neglog_marginal_likelihood(D,x,sigma);     % evaluate objective function
   fprintf('iter = %3d: x = %-10s, neg_log_likelihood = %f\n', ...
